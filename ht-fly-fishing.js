...
/* Get method-aware tackle recommendation — adapts to fly, spin, or bait */
function getMethodAwareRec(water, habitat, method) {
  ...
  // Determine which method we're recommending for
  var m = method || 'fly';

  // Try condition-based engine first (from trout-tackle-knowledge.js)
  if (typeof window.getConditionTackleRec === 'function') {
    var snapshot = (typeof getFlyWeatherSnapshot === 'function') ? getFlyWeatherSnapshot() : {};
    var conditions = {
      temp: snapshot.temp || null,
      wind: snapshot.wind || null,
      period: period,
      season: season,
      waterClarity: 'clear', // default, can be updated by voice reports
      sky: _inferSkyCondition(snapshot.temp, period)
    };
    // Check if coach has reported water clarity
    if (_aiCoachState && _aiCoachState.reports && _aiCoachState.reports.length) {
      var lastReports = _aiCoachState.reports.slice(-3).map(function(r) { return r.text.toLowerCase(); }).join(' ');
      if (lastReports.match(/muddy|murky|dirty|flood/)) conditions.waterClarity = 'murky';
      else if (lastReports.match(/stain|color|off-color|turbid/)) conditions.waterClarity = 'stained';
      else if (lastReports.match(/clear|crystal|gin/)) conditions.waterClarity = 'clear';
    }
    var conditionRec = window.getConditionTackleRec(m, conditions);
    if (conditionRec) {
      tackleRec = conditionRec.primary;
      altTackle = conditionRec.alternate;
    }
  }

  // Fallback: use TROUT_HOTSPOT_FLAVOR arrays if condition engine unavailable
  if (!tackleRec && flavor) {
    var arr;
    if (m === 'bait' && flavor.bait && flavor.bait.length) arr = flavor.bait;
    else if (m === 'spin' && flavor.lures && flavor.lures.length) arr = flavor.lures;
    else if (flavor.flies && flavor.flies.length) arr = flavor.flies;
    if (arr && arr.length) {
      var idx = Math.floor(Math.random() * arr.length);
      tackleRec = arr[idx];
      altTackle = arr[(idx + 1) % arr.length];
    }
  }

  // Set label based on method
  if (m === 'bait') tackleLabel = 'BAIT';
  else if (m === 'spin') tackleLabel = 'LURE';
  else tackleLabel = 'FLY';

  // Method-aware time advice
  var timeAdvice = '';
  if (m === 'bait') {
    if (period === 'early-morning') timeAdvice = 'Dawn is prime bait time. Trout feed aggressively in low light. Get your bait in the water before first light and be patient.';
    else if (period === 'morning') timeAdvice = 'Prime morning feeding window. Trout are active and searching for food. Keep your bait fresh and in the strike zone.';
    else if (period === 'midday') timeAdvice = 'Midday sun pushes trout deep. Fish the deepest pools and shadowed areas. Let your bait soak longer — trout feed slower now.';
    else if (period === 'afternoon') timeAdvice = 'Afternoon activity picks up. Check your bait freshness. If PowerBait is washed out, re-bait. Trout start moving again.';
    else if (period === 'evening') timeAdvice = 'Evening is excellent for bait fishing. Trout move to shallower feeding positions. Fresh bait in runs and riffles now.';
    else timeAdvice = 'Night bait fishing — fish deep pools with strong-scented bait. Nightcrawlers and garlic dough are best in darkness.';
  } else if (m === 'spin') {
    if (period === 'early-morning') timeAdvice = 'Dawn is trophy time for lure fishing. Dark-colored lures create strong silhouettes. Trout are aggressive and will chase.';
    else if (period === 'morning') timeAdvice = 'Prime lure fishing window. Steady retrieves through riffles and runs. Vary your retrieve speed until you find what triggers strikes.';
    else if (period === 'midday') timeAdvice = 'Midday sun — slow down your retrieve. Fish deeper water with heavier lures. Target shaded structure and deep pools.';
    else if (period === 'afternoon') timeAdvice = 'Afternoon activity picking up. Fan-cast to cover water. The feeding window is approaching — trout start moving.';
    else if (period === 'evening') timeAdvice = 'Golden hour for lure fishing. Trout feed aggressively in fading light. Switch to gold or copper blade colors for warm flash.';
    else timeAdvice = 'Night lure fishing — large dark-profile lures retrieved slow near structure. Big predatory trout hunt at night.';
  } else {
    if (period === 'early-morning') timeAdvice = 'Low light — use dark patterns. Trout feed aggressively at dawn. Streamers and dark nymphs are ideal.';
  ...
  return { flyRec: tackleRec, altFly: altTackle, timeAdvice: timeAdvice, hatches: hatches, season: season, period: period, tackleLabel: tackleLabel, method: m };
}

/* Infer sky condition from temp + time of day (until we add cloud_cover to API) */
function _inferSkyCondition(temp, period) {
  // Without cloud cover data, we make educated guesses
  // This is a rough heuristic — will improve when we add cloud_cover API param
  if (period === 'early-morning' || period === 'evening' || period === 'night') return 'any';
  if (temp !== null && temp < 40) return 'overcast'; // cold often = cloudy in MO
  return 'partly-cloudy'; // default assumption
}

/* Backward-compatible alias */
function getTimeAwareFlyRec(water, habitat) {
  return getMethodAwareRec(water, habitat, _getCurrentMethod());
}

/* Get current fishing method from fishFlow */
function _getCurrentMethod() {
  var ff = window._fishFlow;
  return (ff && ff.method) ? ff.method : 'fly';
}
...
function buildMicroPinBriefing(water, zone, habitat) {
  ...
  var rec = getMethodAwareRec(water, habitat, _getCurrentMethod());
  ...
  // Time-aware method-specific recommendation
  var tackleLabel = rec.tackleLabel || 'FLY';
  html += '<div style="font-size:11px;color:#c8e6d5;margin-bottom:4px;line-height:1.4;"><b style="color:#ffe082;">Best ' + tackleLabel + ' (' + escapeHtml(rec.period) + '):</b> ' + escapeHtml(rec.flyRec) + '</div>';
  if (rec.altFly) {
...
function buildFlyStrategyHtml(water, prefs, flow) {
  ...
  if (hatches.length) {
    ...
    if (method === 'bait') {
      hatchEdu = {
        spring: 'Spring warming triggers insect activity and trout feeding. Trout are hungry after winter. PowerBait and nightcrawlers in runs and riffles. This is the best bait season.',
        summer: 'Warm water pushes trout deep. Fish early and late when temps are cooler. Target spring-fed inflows for cold-water refuges. Natural baits like crickets and grasshoppers excel.',
        fall: 'Cooling water triggers aggressive pre-winter feeding. Trout are at peak weight. Scented dough and nightcrawlers in deep pools. Switch to garlic scent as temps drop.',
        winter: 'Cold water means slow metabolism. Downsize everything — smaller bait ball, smaller hook, lighter line. Wax worms under a bobber. Patience is the key.'
      };
    } else if (method === 'spin') {
      hatchEdu = {
        spring: 'Spring activity makes trout aggressive. Spinners in riffles and runs produce well. White and gold are the top colors. Retrieve speed should match water temperature — colder = slower.',
        summer: 'Target dawn and dusk. Small lures in shade and depth. Natural colors in clear water. Find cold-water spring inflows — trout stack up there.',
        fall: 'Trout feed aggressively before winter. Full-speed retrieves through riffles. This is the best spinner fishing season. Cover water quickly — the fish are hungry and willing.',
        winter: 'Slow and deep. The Panther Martin gold is the cold-water king — it spins at the slowest retrieves. Count lures down to the bottom. Tiny jigs drifted naturally also produce.'
      };
    } else {
      hatchEdu = {
  ...
  // --- Time-of-Day Approach (method-aware) ---
  var timeAdviceFly = {
  ...
  var timeAdviceSpin = {
    'early-morning': { title: 'Dawn Strategy', icon: '🌅', text: 'Trophy time. Trout are aggressive in low light. Use dark-colored lures for strong silhouettes. Slow retrieve in deep water. The biggest trout feed at dawn.' },
    'morning': { title: 'Morning Window', icon: '☀️', text: 'Prime lure fishing window. Steady retrieves through riffles and runs. Vary speed until you find the trigger. Fan-cast to cover water methodically.' },
    'midday': { title: 'Midday Approach', icon: '🔆', text: 'Slow down everything. Fish deeper with heavier lures — Kastmaster spoons and weighted jigs. Target shaded pools and structure. Trout hold tight to cover.' },
    'afternoon': { title: 'Afternoon Transition', icon: '🌤', text: 'Activity picks up as the sun angle drops. Switch to gold or copper blade colors. Work seam lines and run transitions. The evening feeding window is approaching.' },
    'evening': { title: 'Evening Prime Time', icon: '🌇', text: 'Golden hour for spinners. Trout move to shallower feeding positions. Use gold blade colors for warm light flash. Cover riffles and tailouts — active fish are there.' },
    'night': { title: 'After Dark', icon: '🌙', text: 'Big dark lures retrieved slowly near structure. Heavy line — 8lb test. Big trout are predators at night. Know the water before dark for safety.' }
  };
  var timeAdviceBait = {
    'early-morning': { title: 'Dawn Strategy', icon: '🌅', text: 'Dawn is the #1 bait fishing window. Get your bait in the water before first light. Trout cruise and feed aggressively now. Fresh bait, best spots, patience.' },
    'morning': { title: 'Morning Window', icon: '☀️', text: 'Prime feeding continues. Leave your bait in prime spots and wait. Check every 15 minutes — fresh PowerBait outperforms washed-out dough by 400%.' },
    'midday': { title: 'Midday Approach', icon: '🔆', text: 'Trout retreat to deep pools and shade. Target the deepest available water. Let your bait soak longer — 20-30 minutes per position. Patience wins in the midday sun.' },
    'afternoon': { title: 'Afternoon Transition', icon: '🌤', text: 'Trout start moving again as sun drops. Re-bait with fresh dough. Consider switching to runs and riffles as fish move from deep pools to feed.' },
    'evening': { title: 'Evening Prime Time', icon: '🌇', text: 'Excellent bait fishing. Trout move to shallower water to feed. Fresh nightcrawler in runs and riffles. Or PowerBait under a bobber at feeding depth.' },
    'night': { title: 'After Dark', icon: '🌙', text: 'Night bait fishing targets big trout. Use strong-scented bait — garlic PowerBait, nightcrawlers. Fish deep pools near structure. Big trout hunt by scent at night.' }
  };
  var timeAdviceMap = method === 'bait' ? timeAdviceBait : method === 'spin' ? timeAdviceSpin : timeAdviceFly;
  var ta = timeAdviceMap[period] || timeAdviceMap['morning'];
  html += '<div class="ht-fly-strategy-block">';
...
function _buildSpotStrategy(habitat, method, wade, skill, water, zone) {
  ...
  // Method-specific casting/presentation direction
  var castMap;
  if (method === 'bait') {
    castMap = {
      riffle: 'Position upstream. Let bait drift naturally through the riffle with just enough weight to bounce the bottom.',
      pool: 'Cast to the deepest section. Let your bait sink to the bottom. Patience — the big fish are in deep pools.',
      run: 'Drift bait through the run naturally. Set your bobber or weight so it travels the entire run length at the right depth.',
      tailout: 'Light presentation in thin water. Cast upstream and let bait drift through. Natural drift is everything.',
      boulder: 'Drop bait into the calm pocket behind each boulder. Let it sit. Trout find it by scent in the still water.'
    };
  } else {
    castMap = {
  ...
  // Method-aware tackle recommendation
  var rec = getMethodAwareRec(water, habitat, method);
...
window.showMissionSummary = function(water, zone, fishFlow) {
  ...
  var topSpot = spots[0];
  var method = fishFlow.method || 'fly';
  var rec = getMethodAwareRec(water, topSpot ? topSpot.habitat : 'riffle', method);
  var totalSpots = spots.length;

  // Method-aware coaching intro
  var methodIntro = '';
  if (method === 'bait') {
    methodIntro = 'Start with ' + (rec.flyRec || 'PowerBait matched to current conditions') + '. ' +
      'Walk to each pin in order. When you reach a pin, I\'ll deploy micro-spots and tell you exactly where to position your bait. ' +
      'Report back to me with voice — tell me what you see, water conditions, and whether you\'re getting bites. I\'ll adjust your bait color and rigging in real time.';
  } else if (method === 'spin') {
    methodIntro = 'Start with ' + (rec.flyRec || 'a white Rooster Tail 1/8oz') + '. ' +
      'Walk to each pin in order. When you reach a pin, I\'ll deploy micro-spots and give you precise casting and retrieve instructions. ' +
      'Report back to me with voice — tell me what you see, whether trout are following, and water conditions. I\'ll update your lure color and technique.';
  } else {
    methodIntro = 'Start with ' + (rec.flyRec || 'your confidence pattern') + '. ' +
      'Walk to each pin in order. When you reach a pin, I\'ll deploy micro-spots and give you precise casting instructions. ' +
      'Report back to me with voice — tell me what you see, what\'s hatching, and what the trout are doing. I\'ll update your strategy in real time.';
  }
...
function _showSpotInfoTray(water, zone, spot, microType, microIdx) {
  ...
  var rec = getMethodAwareRec(water, spot.habitat, _getCurrentMethod());
  var microLabel = _capitalize(microType.replace(/-/g, ' '));
  var method = _getCurrentMethod();
  var tackleLabel = rec.tackleLabel || 'FLY';
  ...
  var microAdviceSpin = {
    'primary-lie': 'This is the main holding position. Trout actively feed here. Cast past this spot and retrieve your lure through it. Vary your retrieve speed — too fast and they won\'t chase, too slow and the blade stalls.',
    'seam-edge': 'The seam between fast and slow water. Trout intercept food on the edge. Cast upstream and let your lure swing through the seam on the retrieve. The swing triggers reflex strikes.',
    'pocket-water': 'Protected pocket behind structure. Trout ambush from here. Short accurate casts — drop your lure right into the pocket. Let it sink 2-3 seconds before a slow retrieve out.',
    'undercut-bank': 'Undercut bank where trout shelter. Cast your lure parallel to the bank and retrieve it right along the edge. Big trout lurk here and will dart out to intercept.',
    'feeding-lane': 'A natural food channel. Cast your lure upstream and retrieve it through the lane. Keep a steady retrieve speed. Trout in feeding lanes are actively hunting.'
  };
  var microAdviceBait = {
    'primary-lie': 'This is the main holding position where trout feed actively. Position your bait right in this zone. Use enough weight to keep it in the strike zone but not so much it anchors unnaturally.',
    'seam-edge': 'The seam between fast and slow water. Trout hold just inside the slow side waiting for food from the fast current. Drift your bait along this line — the current carries it naturally.',
    'pocket-water': 'Protected pocket behind structure. Trout sit here waiting for food to drift past. Drop your bait right into the calm pocket and let it sit. Trout find it by scent.',
    'undercut-bank': 'Undercut bank where big trout shelter. Position your bait tight to the bank edge with minimal weight. Let it drift under the overhang. Trophy trout hide in these spots.',
    'feeding-lane': 'A natural food channel. This is perfect bait water — let the current carry your offering through naturally. Use just enough weight to bounce the bottom. Natural drift is everything.'
  };
  var microAdvice = method === 'bait' ? microAdviceBait : method === 'spin' ? microAdviceSpin : microAdviceFly;
  ...
  el.innerHTML = `
    ...
    <div class="ht-spot-info-body">
      ...
      <div class="ht-spot-info-section">
        <div class="ht-spot-info-label">${method === 'bait' ? 'PRESENTATION' : 'CASTING'}</div>
        <div class="ht-spot-info-text">${escapeHtml(strat.castDirection)}</div>
      </div>
      <div class="ht-spot-info-section">
        <div class="ht-spot-info-label">RECOMMENDED ${tackleLabel}</div>
        <div class="ht-spot-info-text">${escapeHtml(rec.flyRec || (method === 'bait' ? 'Use condition-matched PowerBait or nightcrawler.' : method === 'spin' ? 'Start with a white Rooster Tail 1/8oz.' : 'Match the hatch — observe the water first.'))}</div>
        ${rec.altFly ? '<div class="ht-spot-info-alt">Backup: ' + escapeHtml(rec.altFly) + '</div>' : ''}
      </div>
      <div class="ht-spot-info-section">
        <div class="ht-spot-info-label">${method === 'bait' ? 'POSITIONING' : 'WADING'}</div>
...
function _processCoachReport(report) {
  ...
  // Hatch detection — relevant for all methods but advice differs
  if (lowerReport.match(/hatch|mayfl|caddis|midge|stonefl|spinner|emerger|bwo|pmd|hendrickson|sulphur/)) {
    if (method === 'fly') {
      if (lowerReport.match(/mayfl|bwo|pmd|hendrickson|sulphur/)) {
        newTackleRec = 'Switch to a #16-18 Parachute Adams or BWO emerger. Match the mayfly hatch.';
        response += '🦟 Mayfly activity detected! ';
      } else if (lowerReport.match(/caddis/)) {
        newTackleRec = 'Switch to #14-16 Elk Hair Caddis dry or a #16 beadhead caddis pupa below.';
        response += '🦗 Caddis hatch confirmed! ';
      } else if (lowerReport.match(/midge/)) {
        newTackleRec = 'Go small — #20-24 Zebra Midge or Griffith\'s Gnat. Tiny. Slow. Patient.';
        response += '🪰 Midge activity noted. ';
      } else if (lowerReport.match(/stonefl/)) {
        newTackleRec = 'Big meal — #8-12 Pat\'s Rubber Legs or Golden Stone nymph. Fish the edges.';
        response += '🪨 Stonefly activity! ';
      } else {
        newTackleRec = 'Match what you see hatching. Go one size smaller than what\'s on the water.';
        response += '🦟 Hatch activity reported. ';
      }
    } else if (method === 'spin') {
      response += '🦟 Insect activity detected — trout are feeding actively. ';
      newTackleRec = 'Trout are keyed in on insects — switch to a Trout Magnet in white or pink. The micro-jig mimics a natural insect better than a spinner. Slow, drifting retrieve.';
    } else {
      response += '🦟 Insect activity detected — trout are feeding near the surface. ';
      newTackleRec = 'Trout are feeding on insects — they may ignore bait on the bottom. Switch to a slip bobber rig to suspend your bait at feeding depth (1-3 ft). Or try a natural offering like wax worms that mimics what they\'re eating.';
    }
  }
  ...
  if (lowerReport.match(/ris(e|ing|er)|sip|splash|boil|strike|hit|took|eat|feed/)) {
    if (method === 'fly') {
      ...
      if (!newTackleRec) newTackleRec = 'Fish are rising — go topwater. Try a #16 Parachute Adams or #14 Elk Hair Caddis.';
    } else if (method === 'spin') {
      response += '🐟 Fish are actively feeding — great sign! They\'re aggressive right now. ';
      if (!newTackleRec) newTackleRec = 'Active fish mean faster retrieves work. Speed up slightly. If using a spinner, keep the blade turning. Try a Rapala twitched on top if fish are hitting the surface.';
    } else {
      response += '🐟 Fish are actively feeding — great sign! They\'re in a biting mood! ';
      if (!newTackleRec) newTackleRec = 'Active fish are aggressive — re-bait with fresh PowerBait or a lively worm. Consider switching to a float rig to present bait at the depth where fish are feeding, not on the bottom.';
    }
  }
  ...
  if (lowerReport.match(/nothing|slow|dead|quiet|no fish|no strike|no bite|no luck|not work/)) {
    ...
    if (method === 'fly') {
      newTackleRec = 'Try going deeper — add weight and switch to a #14 beadhead Pheasant Tail or #12 Woolly Bugger (olive). Change your drift line — move 10 feet upstream and try a new angle.';
    } else if (method === 'spin') {
      newTackleRec = 'Time to change it up. First: slow your retrieve way down. Second: change lure color — if using white, try gold or chartreuse. Third: try a deeper running lure like a Kastmaster spoon with a lift-drop retrieve. Fourth: move to different water.';
    } else {
      newTackleRec = 'Four things to try in order: 1) Change bait color — if using chartreuse PowerBait, switch to rainbow or garlic white. 2) Change your depth — try adding a bobber to suspend bait off the bottom. 3) Downsize your hook and bait ball. 4) Move to different water.';
    }
  }
  ...
  if (lowerReport.match(/refuse|short strike|look|follow|inspect|turn away|won.t take/)) {
    if (method === 'fly') {
      ...
      newTackleRec = 'Go smaller — drop down 2 sizes. Try a #18-20 RS2 or a sparse emerger. Extend your tippet to 6X or 7X. Drag-free drift is critical.';
    } else if (method === 'spin') {
      response += '🧐 Fish are following but not committing — they\'re interested. ';
      newTackleRec = 'Three tricks for followers: 1) Slow down your retrieve dramatically at the last moment — the speed change triggers strikes. 2) Switch to a smaller or more natural-colored lure. 3) Try a Trout Magnet jig drifted naturally instead of a spinner. 4) Add 6lb fluorocarbon leader if you haven\'t already.';
    } else {
      response += '🧐 Fish are investigating but not biting — they\'re cautious. ';
      newTackleRec = 'Finicky trout need finesse: 1) Use a smaller bait ball or hook. 2) Switch to 4lb fluorocarbon leader. 3) Try a natural bait (wax worm or nightcrawler piece) instead of dough. 4) Let your bait sit longer — don\'t move it.';
    }
  }
  ...
  if (lowerReport.match(/muddy|murky|stain|dirty|high water|fast|flood/)) {
    if (method === 'fly') {
      ...
      if (!newTackleRec) newTackleRec = 'In dirty water: #10-12 San Juan Worm (red), #8 Woolly Bugger (black), or bright egg patterns. Strip streamers slow.';
    } else if (method === 'spin') {
      response += '🌊 Stained water — switch to high-visibility lures. ';
      if (!newTackleRec) newTackleRec = 'Dirty water demands: 1) Chartreuse or gold lure colors for maximum visibility. 2) Slower retrieve — trout use vibration to find lures when they can\'t see far. 3) Wide-blade spinners like Mepps Aglia that push more water.';
    } else {
      response += '🌊 Stained water — scent becomes your primary attractor. ';
      if (!newTackleRec) newTackleRec = 'Dirty water bait strategy: 1) Switch to fluorescent orange or chartreuse PowerBait for visibility. 2) Use a larger bait ball for more scent release. 3) Garlic-scented dough outperforms standard. 4) Nightcrawlers with strong natural scent work well. 5) Fish the transition where muddy water meets clear.';
    }
  }
  ...
  if (lowerReport.match(/clear|crystal|gin|low water|skinny/)) {
    if (method === 'fly') {
      ...
      if (!newTackleRec) newTackleRec = 'Clear water demands finesse. Long leaders (12-14ft), small flies (#18-22), and a drag-free drift. Keep low.';
    } else if (method === 'spin') {
      response += '💎 Clear water — go natural and subtle. ';
      if (!newTackleRec) newTackleRec = 'Clear water lure tips: 1) Switch to natural colors (brown trout, silver). 2) Use 4lb fluorocarbon leader — invisible in clear water. 3) Cast long and stay low. 4) Try a Trout Magnet in white — most natural-looking lure available.';
    } else {
      response += '💎 Clear water — finesse time. ';
      if (!newTackleRec) newTackleRec = 'Clear water bait tips: 1) Use natural colors — corn yellow, salmon egg red, or green pumpkin PowerBait. 2) Downsize everything: smaller bait ball, thinner leader (4lb), smaller hook (#14). 3) Use fluorocarbon leader. 4) Natural baits (wax worms, salmon eggs) outperform dough in gin-clear water.';
    }
  }
  ...
  if (lowerReport.match(/wind|windy|gust/)) {
    if (method === 'fly') {
      ...
      if (!newTackleRec) newTackleRec = 'In wind: shorten your leader, use heavier flies, and tuck your casts. Double-haul if you can. Fish sheltered banks.';
    } else if (method === 'spin') {
      response += '💨 Windy conditions — this is actually good for lure fishing! ';
      if (!newTackleRec) newTackleRec = 'Wind is your friend! 1) Choppy surface gives you cover — trout can\'t see you. 2) Switch to a heavier Kastmaster spoon that casts through wind. 3) Fish the windward bank where debris and insects blow into the water. 4) Retrieve slightly faster — wind-stirred water makes trout more aggressive.';
    } else {
      response += '💨 Windy conditions — adapt your bait presentation. ';
      if (!newTackleRec) newTackleRec = 'Windy day bait tips: 1) Use a heavier sinker to maintain contact with your bait. 2) Switch to a tight-line bottom rig instead of a bobber — wind makes bobbers uncontrollable. 3) Fish the windward bank where food washes in. 4) Larger bait ball to resist wave action.';
    }
  }

  // Color change request
  if (lowerReport.match(/color|switch|change|try|different/)) {
    if (method === 'bait' && !newTackleRec) {
      var currentRec = getMethodAwareRec(null, 'pool', method);
      newTackleRec = 'Good instinct to rotate colors. ' + (currentRec.altFly || 'Try the opposite end of the color spectrum from what you\'re using. Bright colors → natural, or natural → bright. Also try mixing two colors on one hook.');
      response += '🎨 Color change recommended. ';
    } else if (method === 'spin' && !newTackleRec) {
      newTackleRec = 'Try switching blade color: gold for overcast/stained water, silver for sunny/clear. If the current color family isn\'t working, go to the opposite — bright to natural or natural to bright. Give each color 10-15 casts.';
      response += '🎨 Lure change recommended. ';
    }
  }
  ...
  if (!response) {
    ...
    if (method === 'fly') {
      newTackleRec = 'Keep observing. Watch for changes in hatching, feeding behavior, or water conditions. Report back when you notice something new.';
    } else if (method === 'spin') {
      newTackleRec = 'Keep working the water. Try varying your retrieve speed and depth. Report what you see — if trout follow, refuse, or ignore. I\'ll adjust your strategy.';
    } else {
      newTackleRec = 'Keep your bait fresh and in the strike zone. Check it every 15 minutes. Watch for any changes in trout activity — feeding patterns, movement, or your line. Report back.';
    }
  }

  response += newTackleRec;

  _aiCoachState.currentStrategy = response;
  _aiCoachState.currentFlyRec = newTackleRec;
...
function _showCoachResponse(responseText) {
  ...
  var method = _getCurrentMethod();
  var coachTitle = method === 'bait' ? 'AI BAIT COACH' : method === 'spin' ? 'AI LURE COACH' : 'AI FLY COACH';
  ...
  el.innerHTML = `
    <div class="ht-coach-response-card">
      <div class="ht-coach-response-header">
        ...
        <span class="ht-coach-response-title">${coachTitle}</span>
